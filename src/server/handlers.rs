...
pub async fn get_config(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    Json(get_base_config_json(&config))
}
...
pub async fn get_config_json(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    let config = state.config.read().await; // Acquire read lock
    let mut json_config = get_base_config_json(&config);

    if let Some(obj) = json_config.as_object_mut() {
        obj.insert("providers".to_string(), serde_json::to_value(&config.providers).unwrap());
        obj.insert("models".to_string(), serde_json::to_value(&config.models).unwrap());
    }

    Json(json_config)
}

/// Creates the base configuration JSON value.
fn get_base_config_json(config: &AppConfig) -> serde_json::Value {
    serde_json::json!({
        "server": {
